...
module jsonpatch {
  ...
  var objOps = {
    ...
    move: function (obj, key, tree) {
      ...
      apply(tree, [temp]);
      apply(tree, [
        {op: "remove", path: this.from, isArray: Array.isArray(this)}
      ]);
      apply(tree, [
  ...
  var rootOps = {
    ...
    replace: function (obj) {
      apply(obj, [
        {op: "remove", path: this.path, isArray: Array.isArray(this)}
      ]);
      apply(obj, [
  ...
  var observeOps = {
    ...
    'delete': function (patches:any[], path) { //single quotes needed because 'delete' is a keyword in IE8
      var patch = {
        op: "remove",
        path: path + escapePathComponent(this.name),
        isArray: Array.isArray(this.object[this.name])
      };
      patches.push(patch);
    },
  ...
  function _generate(mirror, obj, patches, path) {
    ...
    for (var t = oldKeys.length - 1; t >= 0; t--) {
      ...
      if (obj.hasOwnProperty(key)) {
        ...
        patches.push({op: "remove", path: path + "/" + escapePathComponent(key), isArray: Array.isArray(obj)});
